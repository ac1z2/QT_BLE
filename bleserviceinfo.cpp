...
QBluetoothUuid BLEServiceInfo::characteristic(const QString &name)
    const{
        ...
        while (i.hasNext()) {
            i.next();
            for( int j=0; j< i.value().characteristics.keys().size(); ++j)
                    {
                            if(i.value().characteristics.values().at(j)==name)
                                   return i.value().characteristics.keys().at(j);
                    }
