...
QObject *BLEDeviceFinder::findByName(const QString &name) const
{
    foreach(QObject *o, m_devices) {
        ...
        QString devName = d->name().toUpper();
        qDebug() << devName;
...
QObject *BLEDeviceFinder::findbyAdress(const QString &uuid) const
{
    foreach(QObject *o, m_devices) {
        BLEDevice *d = qobject_cast<BLEDevice *>(o);
        QString devAdress = d->address().toUpper();
        qDebug() << devAdress;

        if (devAdress.contains(uuid))
            return o;
    }
